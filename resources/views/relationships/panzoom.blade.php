    ...
    d3.json("{{ route('api.relationships.getData') }}").then(members => {
        ...
        const updateLinks = () => {
            ...
            d3.selectAll("li[data-parent]").each(function () {
                ...
                const [x1, y1] = [
                    parentA.getBoundingClientRect().left + parentA.offsetWidth / 2 - wrapperRect.left,
                    parentA.getBoundingClientRect().bottom - wrapperRect.top
                ];
                const [x2, y2] = [
                    childA.getBoundingClientRect().left + childA.offsetWidth / 2 - wrapperRect.left,
                    childA.getBoundingClientRect().top - wrapperRect.top
                ];
                const deltaY = (y2 - y1) * 0.5;
                linksLayer.append("path")
                    .attr("class", "link")
                    .attr("d", `M${x1},${y1} C${x1},${y1 + deltaY} ${x2},${y2 - deltaY} ${x2},${y2}`)
                    .attr("fill", "none")
                    .attr("stroke", "#ccc")
                    .attr("stroke-width", 2)
        ...
        d3.select("#panzoom-container").call(zoomBehavior);
